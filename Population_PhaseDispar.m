function Population_PhaseDispar()
...
if exist('Population_PhaseDispar.mat')
	load('Population_PhaseDispar.mat','Exper');
else
	for n = 1:length(Exper)
		if ~isempty(Exper(n).Test(5).FileData)
			% Selection criteria for PhaseDispar stim
			Exper(n).Test(5).Tun = TuningFunc(Exper(n).Test(5).Trial,'PhaseDisparity_vs_Ori',Exper(n).Test(5).Stim.Duration,unique([Exper(n).Test(5).Trial.condition]));
			Exper(n).Test(5).SelectCrit = false(length(Exper(n).Test(5).Tun),3);
			tic
			for m = 1:length(Exper(n).Test(5).Tun)
				clust_idx = Exper(n).Test(5).Tun(m).clust_idx;
				Exper(n).Test(5).SelectCrit(m,1) = Exper(n).Test(3).Cluster(clust_idx).metrics.noise_overlap<CRIT_CLUSTNOISEOVERLAP;
				Exper(n).Test(5).SelectCrit(m,2) = Exper(n).Test(3).Cluster(clust_idx).metrics.isolation>CRIT_CLUSTISOLATION;
				Exper(n).Test(5).SelectCrit(m,3) = RespCriterion(Exper(n).Test(5).Tun(m));
			end
			toc
		end
	end
	save('Population_PhaseDispar.mat','Exper');
end
SelectCrit=[];
for n=1:length(Exper)
	if ~isempty(Exper(n).Test(5).FileData)
		SelectCrit=cat(1,SelectCrit,Exper(n).Test(5).SelectCrit);
	end
end
...
for n = 1:length(Exper)
	if ~isempty(Exper(n).Test(5).FileData)
		% First discard PhaseDispar Tun
		clust_sel = all(Exper(n).Test(5).SelectCrit,2);
		Exper(n).Test(5).Tun = Exper(n).Test(5).Tun(clust_sel);
		% Next discard RDS dispar, if data exists
		if ~isempty(Exper(n).Test(3).Tun)
			clust_idx = [Exper(n).Test(3).Tun.clust_idx]';
			for m = 1:length(clust_idx)
				thisclust = [Exper(n).Test(3).Tun.clust_idx]'==clust_idx(m);
				TunDispar(m) = Exper(n).Test(3).Tun(thisclust);
			end
			Exper(n).Test(3).Tun = TunDispar;
			clear TunDispar clust_idx thisclust;
		end
	end
end
...
for n = 1:length(Exper)
	if ~isempty(Exper(n).Test(5).FileData)
		Tun = Exper(n).Test(5).Tun;
		for m = 1:length(Tun)
			[PDSI,ori_idx] = PhaseDisparSelectivityIndex(Tun(m));
			[~,chanprim] = NeuronPrimaryChan(Exper(n).Test(5).FileData,Exper(n).Test(5).FileSpike,Tun(m).clust_idx,Exper(n).Test(3).chanpos);
			v = Exper(n).Test(3).Templates(chanprim,:,Tun(m).clust_idx);
			[~,spk_slope,v_norm] = SpikeWave(v,'TreeShrew');
			DDI = DisparDiscrimIndx_Grating(Tun(m));
			Data = [Data; ...
				n Tun(m).clust_idx Tun(m).Anova(ori_idx) PDSI spk_slope(2) DDI ori_idx];
% 			if n==20 || n==8 || n==9 || n==17 || n==19 || n==14 || n==16 || n==6 || n==3
			if true
				SpikeWaveform = cat(1,SpikeWaveform,v_norm);
			end
		end
	end
end

%% Discard cells that had all negative responses
SpikeWaveform = SpikeWaveform(Data(:,4)~=-1,:);
Data = Data(Data(:,4)~=-1,:);

%% Plotting
figure; %PDSI distribution
SIGNIF = Data(:,3)<0.05;
histbin = [0:0.05:1];
PDSI_hist(1,:) = histcounts(Data(SIGNIF,4),histbin);
PDSI_hist(2,:) = histcounts(Data(~SIGNIF,4),histbin);
histcent = histbin(1:end-1)+(histbin(2)-histbin(1))/2;
oh = bar(histcent,PDSI_hist','stacked');
oh(1).FaceColor = 'k'; oh(2).FaceColor = 'w'; box off;
ah = gca; ah.TickDir = 'out'; ah.FontSize=14;
xlabel('PDSI'); ylabel('Num of neurons');
legend('ANOVA p<0.05','ANOVA p>0.05');

figure; %DDI distribution
SIGNIF = Data(:,3)<0.05;
histbin = [0:0.05:1];
DDI_hist(1,:) = histcounts(Data(SIGNIF,6),histbin);
DDI_hist(2,:) = histcounts(Data(~SIGNIF,6),histbin);
histcent = histbin(1:end-1)+(histbin(2)-histbin(1))/2;
...
figure; % PDSI vs DDI
oh = plot(Data(:,4),Data(:,6),'ko'); hold on;
oh.MarkerFaceColor = oh.Color;
xlabel('PDSI'); ylabel('DDI'); box off;
ah=gca; ah.FontSize=14;
[b,bint] = maregress(Data(:,4),Data(:,6));
xx = [0.2 0.8]; hold on;
plot(xx,b(1)+b(2)*xx,'k--');

figure; % Spike waveform
plot([-74:75]/20,SpikeWaveform(Data(:,5)>=0,:)','k'); hold on;
plot([-74:75]/20,SpikeWaveform(Data(:,5)<0,:)','Color',[0 0.4470 0.7410]);
xlabel('Time (ms)'); ylabel('Normalized voltage'); box off;
ah=gca; ah.FontSize=14; xlim([-1 2])

figure; % Cell-type vs PDSI (Cell-type is Fast-spiking or regular-spiking)
type_fs = Data(:,5)<0;
x_fs = randn(sum(type_fs),1)*0.1+1;
x_rs = randn(sum(~type_fs),1)*0.1+2;
oh = plot([x_fs; x_rs],[Data(type_fs,4); Data(~type_fs,4)],'ko'); hold on;
oh.MarkerFaceColor = oh.Color;
ylabel('PDSI'); box off; ah=gca; ah.FontSize=14;
ah.XTick = [1 2]; ah.XTickLabel={'FS','RS'};

figure; % Cell-type vs DDI
oh = plot([x_fs; x_rs],[Data(type_fs,6); Data(~type_fs,6)],'ko'); hold on;
oh.MarkerFaceColor = oh.Color;
ylabel('DDI'); box off;
ah=gca; ah.FontSize=14; ah.XTick=[1 2]; ah.XTickLabel={'FS','RS'};

figure; % PDSI distribution for each ori separately
datajit = randn(size(Data,1),1)*0.1;
oh = plot(Data(:,7)+datajit,Data(:,4),'ko'); hold on;
for n = 1:4
	ddiori(n) = mean(Data(Data(:,7)==n,4));
end
plot(1:4,ddiori,'k-');
oh.MarkerFaceColor = oh.Color;
ylabel('PDSI'); box off; ah=gca; ah.FontSize=14;
ah.XTick = [1:4]; ah.XTickLabel={'0 deg','45 deg','90 deg','135 deg'};
