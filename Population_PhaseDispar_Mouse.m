function Population_PhaseDispar_Mouse()
...
if exist('Population_PhaseDispar_Mouse.mat','file')
	load('Population_PhaseDispar_Mouse.mat','Exper');
else
	for n = 1:length(Exper)
		% Selection critera for PhaseDispar data
		Tun = Exper(n).PhaseDispar.Tun;
		for m = 1:length(Tun)
			mtr = zeros(1,2);
			...
			Exper(n).PhaseDispar.crit_pass(m) = flag_pass;
		end
	end
	save('Population_PhaseDispar_Mouse.mat','Exper');
end

%% Discard units that failed the selection criterion
for n = 1:length(Exper)
	m_sel = find(Exper(n).PhaseDispar.crit_pass);
	Exper(n).PhaseDispar.Tun = Exper(n).PhaseDispar.Tun(m_sel);
	for m = 1:length(m_sel)
		clust_idx = Exper(n).PhaseDispar.Tun(m).clust_idx;
		clust_ori = find([Exper(n).OriTun.Tun.clust_idx] == clust_idx);
		clust_dispar = find([Exper(n).RDSDispar.Tun.clust_idx] == clust_idx);
		if ~isempty(clust_ori)
			OriTun(m) = Exper(n).OriTun.Tun(clust_ori);
		end
		if ~isempty(clust_dispar)
			DispTun(m) = Exper(n).RDSDispar.Tun(clust_dispar);
		end
	end
	Exper(n).OriTun = rmfield(Exper(n).OriTun,'Tun');
	Exper(n).RDSDispar = rmfield(Exper(n).RDSDispar,'Tun');
	for m = 1:length(m_sel)
		Exper(n).OriTun.Tun(m) = OriTun(m);
		Exper(n).RDSDispar.Tun(m) = DispTun(m);
	end
	clear OriTun PhiTun;
end
...
for n = 1:length(Exper)
	...
	for m = 1:length(Tun)
		...
		pos = strfind(Exper(n).PhaseDispar.FiringFile,'firings');
		FiringFile = Exper(n).PhaseDispar.FiringFile(pos:end);
		if n<8
			probe = probe_64M_func;
		else
			probe = probe_64D_func;
		...
		[chanz,chanprim] = NeuronPrimaryChan(Exper(n).PhaseDispar.DigFile,FiringFile,Tun(m).clust_idx,Exper(n).ClustTemplate.ChanPos);
...
% 		if n==7 || n==8 || n==13 || n==14
		if true
			SpikeWaveform = cat(1,SpikeWaveform,v_norm);
		end
		% Inserted to test the shifting of phase disapr tuning with ori
% 		if PDSI>0.2
% 			PlotPhaseDisparityTuning(Tun(m),n);
% 		end
		%%%%%%

	end
	OriTun = Exper(n).OriTun.Tun;
	for m = 1:length(OriTun)
		TunOriBinoc = OriTun(m).Resp(1:3:34,1);
		Rblank = OriTun(m).Resp(end,1);
		[~,~,~,DSI]  = prefDOgOSIgDSI(TunOriBinoc - Rblank);
		DataOri = [DataOri; n OriTun(m).clust_idx DSI];
	end
end
...
% figure;
% DSI_hist = histcounts(DataOri(:,3),histbin);
% oh = bar(histcent,DSI_hist');
% oh.FaceColor = 'k'; box off;
% ah = gca; ah.TickDir = 'out'; ah.FontSize=14;
% xlabel('DSI'); ylabel('Num of neurons');
...
% 	end
	...
	for m = 1:length(Tun)
		...
		[spk_ptratio,spk_slope] = SpikeWave(v);
